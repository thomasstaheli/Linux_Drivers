...
#define HEX0_OFFSET	      0x20 // Offset HEX0, les leds commencent 
								// à 0xFF200020
#define HEX4_OFFSET_FROM_HEX0 0x10 // Offset HEX4 depuis HEX0
#define LEDS_OFFSET	      0x00 // Offset LEDs, les leds commencent 
								// à 0xFF200000
#define KEY_OFFSET	      0x50 // Offset KEYS, les keys commencent 
								// à 0xFF200050
// Interrupts OFFSET
#define KEY_OFFSET_INTERRUPTMASK_REGISTER \
	0x58 // Offset KEYS pour le registre interruptmask
#define KEY_OFFSET_EDGECAPTURE_REGISTER \
	0x5C // Offset KEYS pour le registre edgecapture
 ...
 *
 * Cette fonction met à jour l'affichage des afficheurs 7 segments en fonction
 * du caractère actuellement sélectionné dans `displayed_char`. Elle gère aussi
 * l'affichage du curseur via les LEDs.
 *
 * @param displayed_char Pointeur vers la chaîne contenant les caractères à
 * afficher.
 ...
 * modifier uniquement la partie du registre associée. Si le caractère 
 * à afficher est un espace, l'afficheur correspondant est éteint.
 */
static void update_display(char *displayed_char, uint8_t cursor)
{
	...
	// Masque permettant de modifier uniquement la partie désirée du registre
	uint8_t shift;

	// Si le curseur se trouve sur les hex 5 ou 6, si oui alors il faut 
	// modifier le deuxième registre
...
int main()
{
	...
	key_interrupt_mask =
		(uint8_t *)(fpga_base + KEY_OFFSET_INTERRUPTMASK_REGISTER);
	key_edge_capture =
		(uint8_t *)(fpga_base + KEY_OFFSET_EDGECAPTURE_REGISTER);
	...
	*key_edge_capture = 0xF;
	...
	while (1) {
		...
		// Utilisation de select pour gérer les interruptions
